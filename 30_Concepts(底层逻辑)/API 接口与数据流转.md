...
---
1. HTTP 设计的根本目的是什么？
	答案：在不可靠的网络上，让两个不认识的程序可以可靠地交换信息
2. HTTP 协议就是“寄信规则”
	1. **GET = “问路”**：你只问，不带东西，地址写在信封（URL）上，大家都可以看到。问10次都一样（幂等）。
	2. **POST = “寄包裹”**：你带行李（body）给对方，箱子里可以放很多东西，也可以让对方“改库存”。寄10次结果可能都不一样。
	3. [[为什么 GET 参数放 URL，POST 参数放 Body？]]
		1. GET 广播，只需要
3. 核心区别：
	1. GET ： 无 Body、参数在 URL 后面、适合“查”，可缓存、安全（是**语义约定**，不是技术强制。用 GET 删除数据 technically 可行但违反规范）。
	2. POST ： 有 Body、适合“增/改”，POST响应**可以**被缓存，默认不缓存，需要设置 Cache-Control 头。
4. 补充：HTTP 还有 PUT（全量更新）、DELETE（删除）、PATCH（局部更新）。
5. **Demo（立刻复制到 Postman）**：
	1. 新建 Request
...
3. **补充知识**： 现在很多公司还在用 REST，但 GraphQL 和REST 现共存（一次请求就能拿多给资源，，像“点一桌菜”）。
4. **Demo**：
...
```
5. **服务器解析 JSON 的过程**：收到请求 --> 读取 Body --> 按 JSON 规则逐字符解析 --> 发现非法逗号 --> 返回400
